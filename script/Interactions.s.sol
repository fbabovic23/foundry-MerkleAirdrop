...
contract ClaimAirdrop is Script {
    error Interactions__SignatureLengthInvalid();
    ...
    function splitSignature(bytes memory sig) public pure returns (uint8 r, bytes32 s, bytes32 v) {
        if (sig.length != 65) {
            revert Interactions__SignatureLengthInvalid();
        }
